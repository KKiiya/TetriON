...
public class MultiplayerB : ButtonWrapper {
    ...
    public MultiplayerB(MenuWrapper menu, Vector2 position, string id = "multiplayer", Dictionary<string, InterfaceTextureWrapper> textures = null)
        ...
        _originalTexture = new InterfaceTextureWrapper(skinManager.GetTextureAsset("multiplayer_b"), Vector2.Zero);

        // Load different state textures
        try {
            _clickTexture = new InterfaceTextureWrapper(skinManager.GetTextureAsset("multiplayer_b_click"), Vector2.Zero);
            _hoverTexture = new InterfaceTextureWrapper(skinManager.GetTextureAsset("multiplayer_b_hover"), Vector2.Zero);
            _disabledTexture = new InterfaceTextureWrapper(skinManager.GetTextureAsset("multiplayer_b_disabled"), Vector2.Zero);
        } catch {
