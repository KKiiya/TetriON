...
namespace TetriON.Shared.UI.Menus.MainMenu.Buttons;

public class MultiplayerB : ButtonWrapper
{
    ...
    public MultiplayerB(MenuWrapper menu, Vector2 position, string id = "multiplayer", Dictionary<string, InterfaceTextureWrapper> textures = null)
        : base(menu, position, id, textures)
    {
        // TetriON.DebugLog("MultiplayerB: Constructor started, calling InitializePrimaryConstructor");
        // Initialize ButtonWrapper mouse events first
        InitializePrimaryConstructor();
        // TetriON.DebugLog("MultiplayerB: InitializePrimaryConstructor completed");

        SkinManager skinManager = menu.GetGameSession().GetSkinManager() ?? throw new Exception("SkinManager is null");
        var (success, texture) = skinManager.GetTextureAsset("multiplayer_b");
        ...
        try
        {
            var (successClick, textureClick) = skinManager.GetTextureAsset("multiplayer_b_click");
            if (!successClick) textureClick = texture; // Fallback to original if click texture not found
            var (successHover, textureHover) = skinManager.GetTextureAsset("multiplayer_b_hover");
            if (!successHover) textureHover = texture; // Fallback to original if hover texture not found
            var (successDisabled, textureDisabled) = skinManager.GetTextureAsset("multiplayer_b_disabled");
    ...
    public MultiplayerB(MenuWrapper menu, Vector2 position, string id = "multiplayer", InterfaceTextureWrapper texture = null)
    ...
    {
        // TetriON.DebugLog("MultiplayerB: OnButtonClicked called - switching to click texture");
    ...
    {
        // TetriON.DebugLog("MultiplayerB: OnButtonHoverEnter called - switching to hover texture");
        if (IsEnabled()) SetTexture(_hoverTexture);
    }

    protected override void OnButtonHoverExit()
    {
        // TetriON.DebugLog("MultiplayerB: OnButtonHoverExit called - switching to original texture");
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        {
            // Clear custom event
            OnMultiplayerButtonPressed = null;
        }
