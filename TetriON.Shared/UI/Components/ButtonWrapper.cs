...
namespace TetriON.Shared.UI.Components;
...
public class ButtonWrapper : IDisposable
{
    ...
    {
        _id = id ?? string.Empty;
        _session = menu?.GetGameSession() ?? TetriONGame.Instance?.Session ?? throw new InvalidOperationException("No GameSession available");
        _mouseInput = TetriONGame.Mouse;
