...
using static TetriON.Shared.Logic.Tetromino.Kicks;
...
public class Grid
{
    ...
    private readonly TetriONGame _game;
    ...
    public Grid(TetriONGame game, GameSettings settings, float sizeMultiplier = 2)
    {
        _game = game;
        _settings = settings;
    ...
    private void ApplyPreset(GridPresets.PresetType presetType)
    {
        ...
        for (var x = 0; x < _width; x++)
        {
            ...
            {
                if (preset[y, x]) SetCell(x, y, TetrominoBase.GetTileId("garbage")); // Fixed: use [y, x] to match preset array dimensions
            }
        }
    }
    ...
    public void PlaceTetromino(TetrominoBase tetromino, Point position)
    {
    ...
    private void DrawStaticGrid(SpriteBatch spriteBatch, Point location, Texture2D tiles, int scaledTileSize)
    {
        for (var x = 0; x < _width; x++)
        {
            for (var y = 0; y < _height; y++)
            {
                ...
                var tile = TetrominoBase.GetTileName(color);
                if (string.IsNullOrEmpty(tile)) continue;
                var tilePosition = TetrominoBase.GetTilePosition(color);
    ...
    private void DrawBufferZone(SpriteBatch spriteBatch, Point location, Texture2D tiles, int scaledTileSize)
    {
        ...
        for (var x = 0; x < _width; x++)
        {
            for (var bufferY = 0; bufferY < _bufferZoneHeight; bufferY++)
            {
                ...
                var color = _preAnimationGrid[x][bufferY];
                if (color == EMPTY_CELL) continue;
    ...
    public static Dictionary<string, Point[]> GetWallKicks(bool isI)
    {
        return isI ? KICKS.IKicks : KICKS.Kicks;
    }
    ...
    public Dictionary<string, Point[]> GetLastKick()
    {
        return lastKick;
    }
}
